...
def render(src, fruit, predicted_label, output_path):
    ...
    mask = bayes(original_img, f"{fruit}.txt")
...
def predict(img_src, model_path, fruit, label_file_path):
    ...
    processed_img = preprocess_img(img_src, fruit)
...
def main(img_src: str,
    ...
    try:
        predicted_label = predict(img_src, model_path, fruit, label_file_path)
        render(img_src, fruit, predicted_label, output_path)
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print("Usage: python predict.py <image_path> <model_path> \
            <Fruit> <label_file_path> <output_path>")
        exit(1)
